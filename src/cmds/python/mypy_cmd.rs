...
use crate::core::utils::{print_command_not_found, resolved_command, strip_ansi, tool_exists, truncate};
use anyhow::Result;
use regex::Regex;
...
pub fn run(args: &[String], verbose: u8) -> Result<i32> {
    if !tool_exists("mypy") {
        return Ok(print_command_not_found("mypy"));
    }

    let mut cmd = resolved_command("mypy");
